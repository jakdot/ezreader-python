...
def simulation(sentence):
        ...
        while True:
            try:
                ...
                break
            if float(sim.fixation_point) >= 7 and start_time == 0:
                start_time = sim.time
            if float(sim.fixation_point) >= 12 or float(sim.fixation_point) < 6 and start_time != 0:
                return sim.time - start_time
        return np.nan

simulation_values = [(25, 0.01), (150, 0.01), (25, 0.6), (150, 0.6)]
#simulation_values = [(25, 0.8)]
...
for _ in range(1000):
    ...
    for inttime, intfailure in simulation_values:
        simulation_value = simulation([ez.Word('walked', 159, 0, float(inttime), float(intfailure)), ez.Word('across', 5e03, 0, 25, 0), ez.Word('the', 1e05, 1, 25, 0)])
    ...
    for inttime, intfailure in simulation_values:
        simulation_value = simulation([ez.Word('ambled', 1, 0, float(inttime), float(intfailure)), ez.Word('across', 5e03, 0, 25, 0), ez.Word('the', 1e05, 1, 25, 0)])
        if simulation_value != 0:
